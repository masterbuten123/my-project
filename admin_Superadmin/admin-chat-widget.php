...
<div id="chatWidgetBox" class="card shadow-lg position-fixed bottom-0 end-0 m-4" style="width: 350px; display: none; z-index: 1050;">
    ...
    <div class="card-body p-2" style="height: 400px; overflow-y: auto;">
        <div class="row">
            <div class="col-12" id="userListWrapper">
                <h6>Users</h6>
                <ul id="userList" class="list-group email-list mb-2"></ul>
            </div>
            <div class="col-12">
...
function loadUserList() {
    fetch('admin_chat_backend.php?user_list=1')
        .then(res => res.json())
        .then(data => {
...
function loadChat() {
    ...
    fetch(`admin_chat_backend.php?user_email=${selectedEmail}`)
        .then(res => res.json())
        .then(data => {
...
document.getElementById('sendReply').addEventListener('click', () => {
    ...
    fetch('admin_chat_backend.php', {
        method: 'POST',
        body: JSON.stringify({ message, email: selectedEmail }),
    ...
    .then(data => {
        if (data.success) {
            ...
            loadChat(); // Refresh chat after sending message
        } else {
